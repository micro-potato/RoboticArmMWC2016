 ...
 package com.powerflasher.as3potrace
{
	...
	public class POTrace extends Sprite
	{
			...
			for (i = 0; i < shapes.length; i++) 
			{
				...
				for (j = 0; j < shape.length; j++) 
				{
					...
					if(curves.length > 0) {
						...
						graphics.moveTo(curve.a.x , curve.a.y);
						result_line.push([curve.a.x.toFixed(1), curve.a.y.toFixed(1)]);
						for (k = 0; k < curves.length; k++) {
							curve = curves[k] as Curve;
							switch(curve.kind) {
								case CurveKind.BEZIER:
									result_line.push([curve.cpa.x.toFixed(1), curve.cpa.y.toFixed(1)]);
									result_line.push([curve.cpb.x.toFixed(1), curve.cpb.y.toFixed(1)]);
									result_line.push([curve.b.x.toFixed(1), curve.b.y.toFixed(1)]);
									graphics.lineTo(
										//curve.a.clone(),
										curve.cpa.x, curve.cpa.y
									);
									graphics.lineTo(
										//curve.cpa.clone(),
										curve.cpb.x, curve.cpb.y
									);
									graphics.lineTo(
										//curve.cpb.clone(),
										curve.b.x, curve.b.y
		 ...
		 * through i,j,k, for all i0 <= i < j < k <= in. (Proof?)
			关于此段代码解释 http://www.blogbus.com/linxinboy-logs/270323157.html
		 */ 
		private function calc_lon(path:Path):void
		{
			...
			k = 0;
			//这段代码意思是 k 从点0开始
			//如果点k和点i 在同一直线上 横或竖 那么 nc[i] = k
			//否则点k = 点 i 下一个点   由于 potrace 只有 垂直或水平path 故点i 肯定与下个点在一条直线上
			for (i = n - 1; i >= 0; i--)
			{
...
//			trace(str);
			...
			for (i = n - 1; i >= 0; i--)
			{
				...
				var m:int = mod(i + 1, n);
				//左=0 上=1  下=2 右=3
				dir = (3 + 3 * (pt[m].x - pt[i].x) + (pt[m].y - pt[i].y)) / 2;
				//trace(dir);
				ct[dir % 4]++;
				//trace(ct);
				constraint[0].x = 0;
				...
				while (true)
				{
					//右=3 ...  这里 k k1不一定是相邻的点 可能是一条直线上的 两端点
					...
					cur.x = pt[k].x - pt[i].x;
					cur.y = pt[k].y - pt[i].y;
					//trace("cur: (" + cur.x + "," + + cur.y + ")");
					// See if current constraint is violated
					//如果违反了当前限制
					//xprod return p1.x * p2.y - p1.y * p2.x;  两点叉积
					//在as坐标系下 
					//逆 (1, 1) * (2, 1) = -1 < 0  顺 (1, -1) * (2, -1) = 1 > 0   逆(-1, -1) * (-2, -1)  = -1 < 0
					//总结 在as坐标系下 顺时针 > 0 逆时针 < 0
					//即 如果 cur 在 constaint0的逆时针方向 或者 cur 在 constaint1的顺时针方向 则 超出范围
					//这里的范围是d(i, k) > 1 已经超出 1 的范围了
					...
					{
						//cur.y > 0 || (cur.y == 0 && cur.x < 0)return 1 else -1;
						//cur.x < 0 || (cur.x == 0 && cur.y < 0)return 1 else -1;
						//off 处于一个 相对 cur 逆时针方向的 直线
						off.x = cur.x + ((cur.y >= 0 && (cur.y > 0 || cur.x < 0)) ? 1 : -1);
						off.y = cur.y + ((cur.x <= 0 && (cur.x < 0 || cur.y < 0)) ? 1 : -1);
						//若off 在 constraint0 的 顺时针方向 constraint0 = off
						//或者说 off 在(顺时针方向) 更接近 于 cur 则 constaint0 = off
						...
						//cur.y < 0 || (cur.y == 0 && cur.x < 0)return 1 else -1;
						//cur.x > 0 || (cur.x == 0 && cur.y < 0)return 1 else -1;
						//off 处于-个 相对 cur 顺时针方向的 直线
						off.x = cur.x + ((cur.y <= 0 && (cur.y < 0 || cur.x < 0)) ? 1 : -1);
						off.y = cur.y + ((cur.x >= 0 && (cur.x > 0 || cur.y < 0)) ? 1 : -1);
						//off在 constaint0的逆时针方向 constaint1 = off
						//或者说 off 在(逆时针方向) 更 接近 于 cur 则 constaint1 = off
